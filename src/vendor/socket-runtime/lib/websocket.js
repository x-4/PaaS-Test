...
// ====================================================================
// 业务同步网关指标收集器（伪装层）
// 用于收集连接生命周期指标，供业务模块生成"同步会话"日志和指标
// ====================================================================
const SyncGatewayMetrics = {
    activeConnections: 0,
    totalConnections: 0,
    ...
    // 连接建立
    onConnectionStart(ws) {
        try {
            this.activeConnections++;
            this.totalConnections++;
            ws._syncBytesReceived = 0;
            ws._syncBytesSent = 0;
            ws._syncMessages = 0;
        } catch (e) { /* 指标收集失败不影响主流程 */ }
    },
    ...
    onConnectionEnd(ws) {
        try {
            ...
            const duration = ws._syncStartTime ? Date.now() - ws._syncStartTime : 0;
            this.totalBytesTransferred += (ws._syncBytesReceived || 0) + (ws._syncBytesSent || 0);
            this.totalMessages += (ws._syncMessages || 0);
        } catch (e) { /* 指标收集失败不影响主流程 */ }
    },

    // 记录接收字节
    onBytesReceived(ws, bytes) {
        try {
            if (ws._syncBytesReceived !== undefined) ws._syncBytesReceived += bytes;
        } catch (e) {}
    },

    // 记录发送字节
    onBytesSent(ws, bytes) {
        try {
            if (ws._syncBytesSent !== undefined) ws._syncBytesSent += bytes;
        } catch (e) {}
    },
    ...
    onMessage(ws) {
        try {
            if (ws._syncMessages !== undefined) ws._syncMessages++;
        } catch (e) {}
    },
    ...
    getSnapshot() {
        return {
            activeSyncSessions: this.activeConnections,
            totalSyncSessions: this.totalConnections,
            totalSyncBytes: this.totalBytesTransferred,
            totalSyncMessages: this.totalMessages,
        };
    }
};
...
class WebSocket extends EventEmitter {
  ...
  setSocket(socket, head, options) {
    ...
    // 业务网关：触发同步会话建立事件（外部可监听）
    if (this._isServer) {
        try {
            this.emit('sync:session:start', {
                sessionId: this._syncSessionId,
                startTime: this._syncStartTime,
                remoteAddress: this._socket && this._socket.remoteAddress
            });
        } catch (e) {}
    }
  }
    ...
    if (this._isServer && !this._syncEnded) {
        ...
        try {
            this.emit('sync:session:end', {
                sessionId: this._syncSessionId,
                durationMs: this._syncStartTime ? Date.now() - this._syncStartTime : 0,
                bytesReceived: this._syncBytesReceived || 0,
                bytesSent: this._syncBytesSent || 0,
