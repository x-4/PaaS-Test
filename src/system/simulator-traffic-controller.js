...
const state = {
    ...
    vlessThrottleFactor: 1.0, // VLESS流量导致的降频系数
    lastUpdate: Date.now(),
};
...
// 根据VLESS流量计算智能降频系数
// 原理：VLESS流量大时，回环流量占比自然低，可以少造；VLESS流量小时需要多造
function calculateVlessThrottleFactor() {
    try {
        const { bytesPerSecond, trafficLevel } = getTrafficRate();
        // trafficLevel: 0=无流量, 1=低, 2=中, 3=高
        // bytesPerSecond: 当前VLESS入站速率
        if (bytesPerSecond > 1024 * 1024) {
            // >1MB/s：VLESS流量很大，回环占比低，降频至20%
            return 0.2;
        } else if (bytesPerSecond > 100 * 1024) {
            // >100KB/s：VLESS流量中等，降频至50%
            return 0.5;
        } else if (bytesPerSecond > 10 * 1024) {
            // >10KB/s：VLESS流量较低，降频至80%
            return 0.8;
        } else {
            // <10KB/s：VLESS流量很小或无流量，全速运行（保持伪装基线）
            return 1.0;
        }
    } catch (e) {
        return 1.0;
...
function updateStatus() {
    ...
    // 计算两个降频系数
    state.vlessThrottleFactor = calculateVlessThrottleFactor();
    state.quotaThrottleFactor = calculateQuotaThrottleFactor();

    // 综合降频系数 = 两者乘积
    const combinedFactor = state.vlessThrottleFactor * state.quotaThrottleFactor;
    ...
    if (state.quotaThrottleFactor === 0) {
        ...
        if (state.status !== 'throttled') {
            logger.info(`[SimulatorTraffic] 降频运行：综合系数=${combinedFactor.toFixed(2)}（VLESS=${state.vlessThrottleFactor.toFixed(2)}, 配额=${state.quotaThrottleFactor.toFixed(2)}）`);
...
function getThrottleFactor() {
    return state.vlessThrottleFactor * state.quotaThrottleFactor;
}
...
function getStatus() {
    return {
        ...
        vlessThrottleFactor: state.vlessThrottleFactor.toFixed(2),
...
function getMetrics() {
    const s = getStatus();
    return [
        `# HELP simulator_loopback_bytes_total Total loopback bytes used by business simulators`,
        `# TYPE simulator_loopback_bytes_total counter`,
        `simulator_loopback_bytes_total ${state.totalBytes}`,
        `# HELP simulator_traffic_limit_bytes Quota limit for simulator loopback traffic`,
        `# TYPE simulator_traffic_limit_bytes gauge`,
        `simulator_traffic_limit_bytes ${QUOTA_BYTES}`,
        `# HELP simulator_throttle_factor Current combined throttle factor (0-1)`,
        `# TYPE simulator_throttle_factor gauge`,
        `simulator_throttle_factor ${getThrottleFactor()}`,
        `# HELP simulator_status Simulator traffic controller status (0=running,1=throttled,2=paused)`,
        `# TYPE simulator_status gauge`,
        `simulator_status ${state.status === 'running' ? 0 : state.status === 'throttled' ? 1 : 2}`,
    ].join('\n');
}
